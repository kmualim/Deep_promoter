...
def Conv1D(name, input_dim, output_dim, filter_size, inputs, he_init=True, mask_type=None, stride=1, weightnorm=None, biases=True, gain=1.):
    ...
    # TensorFlow 2.x conv1d expects input shape: [batch, width, channels]
    # Your inputs are [batch, channels, width] (NCHW format)
    # Need to transpose
    inputs_transposed = tf.transpose(inputs, [0, 2, 1])  # [batch, width, channels]
    
    result = tf.nn.conv1d(
        input=inputs_transposed, 
        filters=filters, 
        stride=stride,
        padding='SAME'
    )
    
    # Transpose back to [batch, channels, width]
    ...
    if biases:
        ...
        # Add biases - need to reshape for broadcasting
        # result shape: [batch, channels, width]
        # biases shape: [channels]
        _biases = tf.reshape(_biases, [1, -1, 1])  # [1, channels, 1]
